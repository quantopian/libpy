...
/** Structure for holding the ABI version of the libpy library.
    ...
    @note This must match `libpy.load_library._abi_verson` in the Python support
   library.
 */
struct abi_version {
    int major;
...
/** Check that the ABI of the libpy object is compatible with an extension
   module
    compiled against libpy.

    @return true with a Python exception raised if the ABI versions are
   incompatible.
 */
inline bool ensure_compatible_libpy_abi() {
    if (!compatible_versions(libpy_abi_version, detail::header_libpy_abi_version)) {
        py::raise(PyExc_ImportError)
            << "libpy compiled version is incompatible with the compiled version "
               "of this "
               "extension module\nlibpy version: "
            << libpy_abi_version
